export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      dataImportHint: '支持导入 Sub2API 导出 JSON / JSON.GZ，也支持直接选择 OpenAI / Codex 账号 JSON / JSON.GZ 自动识别并批量生成账号。',
      dataImportWarning: '导入会批量创建账号与代理；若检测到重复账号，将自动更新其凭证。请确认已有数据不会冲突。',
      dataImportFile: '数据文件 / 文件夹',
      dataImportFileHint: '支持拖拽多个 JSON / JSON.GZ 文件，或直接选择文件 / 文件夹。系统会自动批量识别 Sub2API 导出和 OpenAI/Codex 账号 JSON。',
      dataImportButton: '开始导入',
      dataImporting: '导入中...',
      dataImportSelectFile: '请选择数据文件或文件夹',
      dataImportChooseFolder: '选择文件夹',
      dataImportSelectedFiles: '已选择 {count} 个文件',
      dataImportNoValidFiles: '未发现可导入的 JSON / JSON.GZ 文件',
      dataImportParseFailed: '数据解析失败',
      dataImportUnsupportedFormat: '暂不支持该 JSON 格式，请选择 Sub2API 导出文件或 OpenAI/Codex 账号 JSON。',
      dataImportGzipUnsupported: '当前浏览器不支持解压 .json.gz 文件，请改用较新的 Chromium 浏览器，或先手动解压后再导入。',
      dataImportFailed: '数据导入失败',
      dataImportDetected: '已识别导入格式',
      dataImportDetectedSummary: '{format} · 文件 {file_count} 个 · 账号 {account_count} 个 · 代理 {proxy_count} 个',
      ...
      dataImportCompletedWithErrors: '导入完成但有错误：账号创建 {account_created}，更新 {account_updated}，失败 {account_failed}；代理失败 {proxy_failed}',
      ...
      oauth: {
        ...
        openai: {
          ...
          refreshTokenDesc: '输入您已有的 OpenAI Refresh Token，或直接粘贴包含 refresh_token 的 JSON 导出内容；支持批量导入，系统会自动验证并创建账号。',
          refreshTokenPlaceholder: '粘贴 OpenAI Refresh Token 或 JSON 导出内容...\n支持多个，每行一个或整个 JSON 数组',
