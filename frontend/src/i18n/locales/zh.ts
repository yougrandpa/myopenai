export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      dataImportHint: '支持导入 Sub2API 导出 JSON，也支持直接选择 OpenAI / Codex 账号 JSON 自动识别并批量生成账号。',
      dataImportWarning: '导入会批量创建账号与代理；若检测到重复账号，将自动更新其凭证。请确认已有数据不会冲突。',
      dataImportFile: '数据文件 / 文件夹',
      dataImportFileHint: '支持拖拽多个 JSON 文件，或直接选择文件 / 文件夹。系统会自动批量识别 Sub2API 导出和 OpenAI/Codex 账号 JSON。',
      dataImportButton: '开始导入',
      dataImporting: '导入中...',
      dataImportSelectFile: '请选择数据文件或文件夹',
      dataImportChooseFolder: '选择文件夹',
      dataImportSelectedFiles: '已选择 {count} 个文件',
      dataImportNoValidFiles: '未发现可导入的 JSON 文件',
